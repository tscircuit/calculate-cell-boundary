...
const App: React.FC = () => {
	const [cellContents, setCellContents] = useState<CellContent[]>(initialCellContents);
	...
	const [draggingCell, setDraggingCell] = useState<{ index: number; offsetX: number; offsetY: number } | null>(null);
	...
	const handleMouseMove = useCallback((event: MouseEvent) => {
		...
		setCellContents((prevCells) =>
			prevCells.map((c, i) =>
				i === draggingCell.index ? { ...c, minX: newMinX, minY: newMinY, maxX: newMaxX, maxY: newMaxY } : c,
			),
		);
	}, [draggingCell, cellContents]); // cellContents is needed here
				...
				<g transform={`translate(0, ${SVG_HEIGHT}) scale(1, -1)`}>
					...
					{cellContents.map((cell, index) => (
						<rect
							...
							fill={draggingCell?.index === index ? RECT_DRAG_FILL_COLOR : RECT_FILL_COLOR}
							stroke="black"
							strokeWidth="1"
			...
			<div className="json-io-container">
				<div className="json-io-column">
					...
					<textarea
						...
						rows={10}
					/>
					<button onClick={handleLoadSceneFromJson} style={{alignSelf: "flex-start", marginTop: "5px"}}>Load/Update Scene from JSON</button>
				</div>
				<div className="json-io-column">
					<label htmlFor="boundaries-output-json">Calculated Boundaries (JSON):</label>
					<textarea id="boundaries-output-json" value={boundariesJsonOutput} readOnly rows={10} />
				</div>
			</div>
		</div>
	);
