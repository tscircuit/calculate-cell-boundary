...
const RectMergeVisualization = () => {
  ...
  const [subdivisionLines, setSubdivisionLines] = useState({
    horizontal: [],
    vertical: [],
  })
  ...
  const generateRandomLines = () => {
    ...
    for (let i = 1; i < NUM_LINES; i++) {
      horizontal.push(
        (i / NUM_LINES) * GRID_SIZE +
          (Math.random() - 0.5) * ((GRID_SIZE / NUM_LINES) * 0.6),
      )
      vertical.push(
        (i / NUM_LINES) * GRID_SIZE +
          (Math.random() - 0.5) * ((GRID_SIZE / NUM_LINES) * 0.6),
  ...
  const createInitialGrid = () => {
    if (
      subdivisionLines.horizontal.length === 0 ||
      subdivisionLines.vertical.length === 0
    ) {
      return []
    }
    ...
    for (let row = 0; row < subdivisionLines.horizontal.length - 1; row++) {
      for (let col = 0; col < subdivisionLines.vertical.length - 1; col++) {
        const rect = {
          ...
          width:
            subdivisionLines.vertical[col + 1] - subdivisionLines.vertical[col],
          height:
            subdivisionLines.horizontal[row + 1] -
            subdivisionLines.horizontal[row],
          row,
          col,
          merged: false,
          groupId: null,
  ...
  const generateSteps = () => {
    ...
    keyPoints.forEach((point, index) => {
      const containingRect = rectsWithMerged.find((rect) =>
    ...
    steps.push({
      ...
      description:
        "Rectangles containing key points are marked as merged (different colors)",
      highlightedRect: null,
    })
    ...
    const rectDistances = unmergedRects
      .map((rect) => {
        const distances = keyPoints.map((point) =>
    ...
    rectDistances.forEach(({ rect: rectRef, distance }) => {
      ...
      const adjacentMerged = currentRects.filter(
        (otherRect) => otherRect.merged && areAdjacent(currentRect, otherRect),
      )
      ...
      <div className="p-6 max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          <div className="text-center">
            Generating irregular grid and key points...
          </div>
        </div>
      </div>
    )
            ...
            <g transform="translate(20, 20)">
              ...
              {currentStep.rects.map((rect) => (
                <rect
                  ...
                  fill={rect.color}
                  stroke={
                    currentStep.highlightedRect === rect.id ? "#333" : "#999"
                  }
                  strokeWidth={
                    currentStep.highlightedRect === rect.id ? "3" : "1"
                  }
